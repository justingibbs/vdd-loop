...
*Agent-generated interpretation of `dispute-summary.feature` **and**
`evaluations/dispute-summary.rubric.md`. This feature spans both verification
layers, so requirements trace to a `.feature` scenario, a rubric criterion, or
both.*

*This example keeps its `specs/` directory intentionally light — a requirements
file and a verification report, no separate `plan.md`/`tasks.md`. Per VDD design
decision #4, the agent has full freedom here and includes only what's useful; a
...
- **Evaluation** (judgment): the summary is faithful, neutral, complete enough to
  route on, and concise. These can't be asserted — they're scored.
...
a side — which is exactly what the rubric guards.
...
| R5 | The summary represents only facts present in the source. | rubric **E1** (Faithfulness ≥ 4) |
| R6 | The summary assigns no blame and attributes claims. | rubric **E2** (Neutrality = PASS) |
| R7 | The summary carries the routing facts: what, amount, date, reason. | rubric **E3** (Completeness ≥ 4) |
| R8 | The summary is dense, not merely under the word cap. | rubric **E4** (Conciseness ≥ 3) |
